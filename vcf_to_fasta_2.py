...
for record in vcf_reader:
    ...
    if isinstance(record.ALT[0], pyvcf.model._SV) or isinstance( record.ALT[0], pyvcf.model._Substitution ):
        ...
        if record.ALT[0].orientation:
            seq1, masked_seq1 = get_seq(record.CHROM, record.POS+offset, record.POS+offset+flank, -1)
        else:
            seq1, masked_seq1 = get_seq(record.CHROM, record.POS-flank-offset, record.POS-offset, 1)
        if not seq1:
            continue
        if record.ALT[0].remoteOrientation:
            seq2, masked_seq2 = get_seq(record.ALT[0].chr, record.ALT[0].pos+offset, record.ALT[0].pos+offset+flank, 1)
        else:
            seq2, masked_seq2 = get_seq(record.ALT[0].chr, record.ALT[0].pos-offset-flank, record.ALT[0].pos-offset, -1)
        if not seq2:
            continue
        sys.stderr.write("\tSucces\n")
        print(">"+record.ID)
        print( masked_seq1+"[]"+masked_seq2 )
